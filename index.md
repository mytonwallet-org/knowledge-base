...
- **balances** — a list of strings in the format `slug:amount` or `slug:amount:valueInBaseCurrency`. `slug` matches a wallet token slug, `amount` is the raw
  integer (bigint) balance, and the optional third field is the token's value in the user's `baseCurrency` (already computed — use this directly to avoid bignum
  arithmetic). For example, `["sol:181980890:16.21","solana-4k3dyjzvzp:2918853:1.72"]` means SOL raw balance `181980890` worth ~16.21 in base currency, and RAY
  raw balance `2918853` worth ~1.72. When the third field is present, **prefer it** over recomputing from raw balance × priceUsd — it accounts for current
  exchange rates. When it's absent (older clients), divide raw balance by `10^decimals` and multiply by `priceUsd` (then convert from USD to `baseCurrency` if
  different). Always show users human-readable values, never raw integers. 
